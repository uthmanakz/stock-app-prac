pipeline{
    ...
    stages{
        ...
        stage('Running app-playbook') {
            steps{
                script{
                    sshagent (credentials: ['SSH_PRIVATE_KEY']) {
                        ...
                        export AWS_SECRET_ACCESS_KEY=${AWS_SECRET_ACCESS_KEY} ;
                        cd stock-app-inventory-prac- ;
                        ansible-playbook -i aws_ec2.yml ../stock-app-playbook-prac/app-playbook.yml"
                        '''
                    }
                }
            }
         ...
         stage('Running app-exporter-playbook') {
            steps{
                script{
                    sshagent (credentials: ['SSH_PRIVATE_KEY']) {
                        sh'''
            ...
            steps{
                script{
                    sshagent (credentials: ['SSH_PRIVATE_KEY']) {
                        ...
                        export AWS_SECRET_ACCESS_KEY=${AWS_SECRET_ACCESS_KEY} ;
                        cd stock-app-inventory-prac- ;
                        ansible-playbook -i aws_ec2.yml ../stock-app-playbook-prac/web-exporter-playbook.yml"
                        '''
                    }
                }
            }
            ...
            steps{
                script{
                    sshagent (credentials: ['SSH_PRIVATE_KEY']) {
                        ...
                        export AWS_SECRET_ACCESS_KEY=${AWS_SECRET_ACCESS_KEY} ;
                        cd stock-app-inventory-prac- ;
                        ansible-playbook -i aws_ec2.yml ../stock-app-playbook-prac/monitoring-playbook.yml"
                        '''
                    }
                }
            }
